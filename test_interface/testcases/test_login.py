...
class TestLogin:
    excel = HandleExcel("login_cases")  # 创建HandleExcel对象
    ...
    @allure.title('login-{case['"title"']}')
    @pytest.mark.parametrize("case", cases)  # case 是对象，因cases列表中包含的是对象
    def test_login(self, case):
        login_url = do_yaml.read("api", "host") + case["url"]  # 构建完成的接口地址
        login_params = HandleParam.do_param(case["data"])  # 取出需要传递的参数（参数化之后的需要替换为实际的）
        login_res = self.do_request.send(login_url,
                                            data=login_params)  # 发起请求，并把请求结果转成json,赋值给register_res
        ...
        try:
            assert json.loads(case["expected"],encoding="utf8").get("code") == login_res_dict["code"]
            assert json.loads(case["expected"], encoding="utf8").get("msg") == login_res_dict["msg"]
            ...
            # 把响应报文写入excel
            self.excel.write_data(row=case["case_id"] + 1, column=do_yaml.read("excel", "actual_col"), value=login_res.text)
